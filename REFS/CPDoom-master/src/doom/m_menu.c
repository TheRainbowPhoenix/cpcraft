...
void M_ReadSaveStrings(void)
{
    ...
    for (i = 0;i < load_end;i++)
    {
        if (M_CheckParm("-cdrom"))
            ...
            sprintf(name,SAVEGAMENAME"%d",i);

        void *data;
        uint32_t size;
        int ret = I_McsRead(name, &data, &size);
        ...
        memcpy(savegamestrings[i], data, SAVESTRINGSIZE);
        LoadMenu[i].status = 1;
    }
}
...
void M_LoadSelect(int choice)
{
	...
	sprintf(name,SAVEGAMENAME"%d",choice);
    G_LoadGame (name);
    M_ClearMenus ();
}
...
boolean M_Responder (event_t* ev)
{
	...
	{
	    mouseypos += ev->data3;
	    if (mouseypos < lasty-30)
	    {
		ch = KEY_DOWNARROW;
		mousewait = I_GetTime() + 5;
		mouseypos = lasty -= 30;
	    }
	    else if (mouseypos > lasty+30)
	    {
		ch = KEY_UPARROW;
		mousewait = I_GetTime() + 5;
		mouseypos = lasty += 30;
	    ...
	    mousexpos += ev->data2;
	    if (mousexpos < lastx-30)
	    {
		ch = KEY_LEFTARROW;
		mousewait = I_GetTime() + 5;
		mousexpos = lastx -= 30;
	    }
	    else if (mousexpos > lastx+30)
	    {
		ch = KEY_RIGHTARROW;
		mousewait = I_GetTime() + 5;
		mousexpos = lastx += 30;
