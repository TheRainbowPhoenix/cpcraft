...
void updateEntitys() {
  for (int i = 0; i < entityLength; i++) {
    if (entityList[i].active == true) {
      ...
      {
        // state 0=standing still, 1=walking, 2=grazing, 3=following player,
        // 4=running, 5=just hit
        int objectIndex = entityList[i].locationOnAllObj;
        int objectIndex2 =
            entityList[entityList[i].extraEntity].locationOnAllObj;
        ...
        if (chunk2DActive[chunkX + chunkZ * TOTALCHUNKWIDTH] == true) {
          ...
          if (entityList[i].state == 5) {
            ...
            color_t color1[28] = {
                ...
                0xe73c, 0xe73c, 0xe73c, 0xe73c, 0xe73c, 0xe73c, 0xe73c,
                0xe73c, 0xe73c, 0xe73c, 0xe73c, 0xe73c, 0xe73c, 0xe73c,
                0xe73c, 0xe73c, 0xe73c, 0xe73c, 0xe73c, 0xe73c, 0x838c,
            };
          ...
          if (entityList[i].state != 4) {
            if (entityList[i].moveTimer < 0) {
              ...
              if (randomNumber < 4) {
                ...
                entityList[i].moveTimer =
                    generate_random(RTC_GetTicks() + i + 1, 4) + 2;

                Vector3B vd1[48] = {
                    {-4, 7, -6}, {-4, 7, 6},  {-4, 13, -6}, {-4, 13, 6},
                    {4, 7, -6},  {4, 7, 6},   {4, 13, -6},  {4, 13, 6},
                    {-2, 10, 8}, {-2, 10, 4}, {-2, 14, 4},  {-2, 14, 8},
                    {2, 10, 8},  {2, 10, 4},  {2, 14, 4},   {2, 14, 8},
                    {-3, 0, 2},  {-3, 0, 5},  {-3, 7, 2},   {-3, 7, 5},
                    {-3, 0, -6}, {-3, 0, -3}, {-3, 7, -6},  {-3, 7, -3},
                    {-1, 0, 2},  {-1, 0, 5},  {-1, 7, 2},   {-1, 7, 5},
                    {-1, 0, -6}, {-1, 0, -3}, {-1, 7, -6},  {-1, 7, -3},
                    {1, 0, 2},   {1, 0, 5},   {1, 7, 2},    {1, 7, 5},
                    {1, 0, -6},  {1, 0, -3},  {1, 7, -6},   {1, 7, -3},
                    {3, 0, 2},   {3, 0, 5},   {3, 7, 2},    {3, 7, 5},
                    {3, 0, -6},  {3, 0, -3},  {3, 7, -6},   {3, 7, -3},
                ...
                for (int j = 0; j < 48; j++) {
                  allObj[objectIndex].verticesIPC[j].x =
                      vd1[j].x + allObj[objectIndex].position.x * 10;
                  allObj[objectIndex].verticesIPC[j].y =
                      vd1[j].y + allObj[objectIndex].position.y * 10;
                  allObj[objectIndex].verticesIPC[j].z =
                      vd1[j].z + allObj[objectIndex].position.z * 10;
                }

                Vector3I center = {allObj[objectIndex].position.x * 10,
                                   allObj[objectIndex].position.y * 10 + 6,
                                   allObj[objectIndex].position.z * 10};
                addRotateObjectNS(objectIndex, objectIndex, center, 0,
                                  90 * (randomNumber - 1));

                Vector3B vd2[48] = {
                    {-3, 8, -5}, {-3, 8, 5},  {-3, 12, -5}, {-3, 12, 5},
                    {3, 8, -5},  {3, 8, 5},   {3, 12, -5},  {3, 12, 5},
                    {-2, 10, 8}, {-2, 10, 4}, {-2, 14, 4},  {-2, 14, 8},
                    {2, 10, 8},  {2, 10, 4},  {2, 14, 4},   {2, 14, 8},
                    {-3, 0, 2},  {-3, 0, 5},  {-3, 8, 2},   {-3, 8, 5},
                    {-3, 0, -6}, {-3, 0, -3}, {-3, 8, -6},  {-3, 8, -3},
                    {-1, 0, 2},  {-1, 0, 5},  {-1, 8, 2},   {-1, 8, 5},
                    {-1, 0, -6}, {-1, 0, -3}, {-1, 8, -6},  {-1, 8, -3},
                    {1, 0, 2},   {1, 0, 5},   {1, 8, 2},    {1, 8, 5},
                    {1, 0, -6},  {1, 0, -3},  {1, 8, -6},   {1, 8, -3},
                    {3, 0, 2},   {3, 0, 5},   {3, 8, 2},    {3, 8, 5},
                    {3, 0, -6},  {3, 0, -3},  {3, 8, -6},   {3, 8, -3},
                ...
                for (int j = 0; j < 48; j++) {
                  allObj[objectIndex2].verticesIPC[j].x =
                      vd2[j].x + allObj[objectIndex2].position.x * 10;
                  allObj[objectIndex2].verticesIPC[j].y =
                      vd2[j].y + allObj[objectIndex2].position.y * 10;
                  allObj[objectIndex2].verticesIPC[j].z =
                      vd2[j].z + allObj[objectIndex2].position.z * 10;
                }

                addRotateObjectNS(objectIndex2, objectIndex2, center, 0,
                                  90 * (randomNumber - 1));
          ...
          int playerDistance =
              calculateDistance3D((int)allObj[objectIndex].position.x,
                                  (int)allObj[objectIndex].position.y,
                                  (int)allObj[objectIndex].position.z,
                                  (int)PPosX, (int)PPosY, (int)PPosZ);
          ...
          if (entityList[i].state == 3) {
            ...
            if (entityList[i].isFed ==
                true) // look for other fed sheeps around to breed with
            {
              for (int j = 0; j < entityLength; j++) {
                if (entityList[j].active == true) {
                  if (entityList[j].type == 1 && entityList[j].isFed == true &&
                      i != j) {
                    float difX =
                        allObj[entityList[j].locationOnAllObj].position.x -
                        allObj[objectIndex].position.x;
                    float difZ =
                        allObj[entityList[j].locationOnAllObj].position.z -
                        allObj[objectIndex].position.z;

                    float difTotal = mysqrt(difX * difX + difZ * difZ);

                    if (difTotal < 3) {
                      makeSheep2((int)allObj[objectIndex].position.x,
                                 (int)allObj[objectIndex].position.y,
                                 (int)allObj[objectIndex].position.z);
            ...
            if (difTotal > 2) {
              bool hasBlockBelow =
                  checkCollision(allObj[objectIndex].position.x,
                                 allObj[objectIndex].position.y - 0.10 -
                                     entityList[i].velocityY * deltaTime,
                                 allObj[objectIndex].position.z);
              bool hasBlockOn =
                  checkCollision(allObj[objectIndex].position.x,
                                 allObj[objectIndex].position.y + 0.25 -
                                     entityList[i].velocityY * deltaTime,
                                 allObj[objectIndex].position.z);
              bool hasBlockAbove =
                  checkCollision(allObj[objectIndex].position.x,
                                 allObj[objectIndex].position.y + 1.25 -
                                     entityList[i].velocityY * deltaTime,
                                 allObj[objectIndex].position.z);
              bool hasBlockInFont = checkCollision(
                  allObj[objectIndex].position.x + rotationForw.x / 2,
                  allObj[objectIndex].position.y + 0.25,
                  allObj[objectIndex].position.z + rotationForw.y / 2);
              bool hasBlockInFontAbove = checkCollision(
                  allObj[objectIndex].position.x + rotationForw.x / 2,
                  allObj[objectIndex].position.y + 1.25,
                  allObj[objectIndex].position.z + rotationForw.y / 2);

              bool isInWater = checkIfInWater(allObj[objectIndex].position.x,
                                              allObj[objectIndex].position.y,
                                              allObj[objectIndex].position.z);
              ...
              if (hasBlockInFont == false) {
                moveObject(objectIndex, rotationForw.x * deltaTimeNoSlow, 0,
                           rotationForw.y * deltaTimeNoSlow);
                moveObject(objectIndex2, rotationForw.x * deltaTimeNoSlow, 0,
                           rotationForw.y * deltaTimeNoSlow);
              } else {
              ...
              if (hasBlockBelow == false) {
                if (isInWater == true) {
                  ...
                  moveObject(objectIndex, 0,
                             entityList[i].velocityY * deltaTime, 0);
                  moveObject(objectIndex2, 0,
                             entityList[i].velocityY * deltaTime, 0);
                } else {
                  entityList[i].fallingSince = allObj[objectIndex].position.y;
          ...
          if (entityList[i].state == 4) {
            if (entityList[i].moveTimer < 0) {
              ...
              if (randomNumber == 0)
                ...
                entityList[i].moveTimer =
                    (float)(generate_random(RTC_GetTicks() + i + 2, 10)) / 10 +
                    0.5f;
            ...
            bool hasBlockBelow =
                checkCollision(allObj[objectIndex].position.x,
                               allObj[objectIndex].position.y - 0.1 -
                                   entityList[i].velocityY * deltaTime,
                               allObj[objectIndex].position.z);
            bool hasBlockOn =
                checkCollision(allObj[objectIndex].position.x,
            ...
            bool hasBlockInFont =
                checkCollision(allObj[objectIndex].position.x +
                                   rotationForw[entityList[i].state2].x / 2,
                               allObj[objectIndex].position.y + 0.25,
                               allObj[objectIndex].position.z +
                                   rotationForw[entityList[i].state2].y / 2);
            bool hasBlockInFontAbove =
                checkCollision(allObj[objectIndex].position.x +
                                   rotationForw[entityList[i].state2].x / 2,
                               allObj[objectIndex].position.y + 1.25,
                               allObj[objectIndex].position.z +
            ...
            Vector2I newPos = {.x = allObj[objectIndex].position.x +
                                    rotationForw[entityList[i].state2].x,
                               .y = allObj[objectIndex].position.z +
                                    rotationForw[entityList[i].state2].y};
            if (newPos.x < 0 || newPos.x > WIDTH * TOTALCHUNKWIDTH ||
                newPos.y < 0 || newPos.y > WIDTH * TOTALCHUNKWIDTH) {
              entityList[i].state = 0;
              entityList[i].moveTimer = -1;
              continue;
            }
            ...
            if (hasBlockInFont == false) {
              moveObject(
                  objectIndex,
                  rotationForw[entityList[i].state2].x * deltaTimeNoSlow, 0,
                  rotationForw[entityList[i].state2].y * deltaTimeNoSlow);
              moveObject(
                  objectIndex2,
                  rotationForw[entityList[i].state2].x * deltaTimeNoSlow, 0,
                  rotationForw[entityList[i].state2].y * deltaTimeNoSlow);
            ...
            if (hasBlockBelow == false) {
              if (isInWater == true) {
                ...
                moveObject(objectIndex, 0, entityList[i].velocityY * deltaTime,
                           0);
                moveObject(objectIndex2, 0, entityList[i].velocityY * deltaTime,
                           0);
              } else {
                entityList[i].fallingSince = allObj[objectIndex].position.y;
          ...
          if (entityList[i].state == 2) {
            if (entityList[i].moveTimer < 0) {
              if (blocks[chunkX + chunkZ * TOTALCHUNKWIDTH]
                        [posX + posZ * WIDTH + (posY - 1) * WIDTH * WIDTH] ==
                  1) {
                entityList[i].hasFur = true;
                allObj[objectIndex2].active = true;
                blocks[chunkX + chunkZ * TOTALCHUNKWIDTH]
                      [posX + posZ * WIDTH + (posY - 1) * WIDTH * WIDTH] = 2;
          ...
          if (entityList[i].health < 1) {
            if (entityList[i].hasFur == true)
              makeItem((int)(allObj[objectIndex].position.x * 10),
                       (int)(allObj[objectIndex].position.y * 10),
                       (int)(allObj[objectIndex].position.z * 10), 0, 0, 0, 95);
      ...
      {
        // state 0=standing still, 1=walking, 3=following player, 4=running,
        // 5=just hit
        int objectIndex = entityList[i].locationOnAllObj;
        ...
        if (chunk2DActive[chunkX + chunkZ * TOTALCHUNKWIDTH] == true) {
          ...
          if (entityList[i].state != 4) {
            if (entityList[i].moveTimer < 0) {
              ...
              if (randomNumber < 4) {
                ...
                Vector3B vd[48] = {
                    {-3, 6, 10},  {3, 6, 10},  {-3, 6, 4},   {3, 6, 4},
                    {-3, 11, 10}, {3, 11, 10}, {-3, 11, 4},  {3, 11, 4},
                    {-4, 4, 5},   {4, 4, 5},   {-4, 4, -6},  {4, 4, -6},
                    {-4, 10, 5},  {4, 10, 5},  {-4, 10, -6}, {4, 10, -6},
                    {-4, 0, 2},   {-1, 0, 2},  {-4, 4, 2},   {-1, 4, 2},
                    {-4, 0, 5},   {-1, 0, 5},  {-4, 4, 5},   {-1, 4, 5},
                    {1, 0, 2},    {4, 0, 2},   {1, 4, 2},    {4, 4, 2},
                    {1, 0, 5},    {4, 0, 5},   {1, 4, 5},    {4, 4, 5},
                    {-4, 0, -7},  {-1, 0, -7}, {-4, 4, -7},  {-1, 4, -7},
                    {-4, 0, -4},  {-1, 0, -4}, {-4, 4, -4},  {-1, 4, -4},
                    {1, 0, -7},   {4, 0, -7},  {1, 4, -7},   {4, 4, -7},
                    {1, 0, -4},   {4, 0, -4},  {1, 4, -4},   {4, 4, -4},
                };

                for (int j = 0; j < 48; j++) {
                  allObj[objectIndex].verticesIPC[j].x =
                      vd[j].x + allObj[objectIndex].position.x * 10;
                  allObj[objectIndex].verticesIPC[j].y =
                      vd[j].y + allObj[objectIndex].position.y * 10;
                  allObj[objectIndex].verticesIPC[j].z =
                      vd[j].z + allObj[objectIndex].position.z * 10;
                }
          ...
          if (entityList[i].state == 3) {
                ...
                true) // look for other fed sheeps around to breed with
            {
              for (int j = 0; j < entityLength; j++) {
                if (entityList[j].active == true) {
                  if (entityList[j].type == 2 && entityList[j].isFed == true &&
                      i != j) {
                    float difX =
                        allObj[entityList[j].locationOnAllObj].position.x -
                        allObj[objectIndex].position.x;
                    ...
                    if (difTotal < 3) {
                      makePig((int)allObj[objectIndex].position.x,
                              (int)allObj[objectIndex].position.y,
                              (int)allObj[objectIndex].position.z);
                      entityList[j].isFed = false;
                      entityList[i].isFed = false;
          ...
          if (entityList[i].health < 1) {
            makeItem((int)(allObj[objectIndex].position.x * 10),
                     (int)(allObj[objectIndex].position.y * 10),
                     (int)(allObj[objectIndex].position.z * 10), 0, 0, 0, 98);
            if (generate_random(RTC_GetTicks() + i, 4) == 0)
              makeItem((int)(allObj[objectIndex].position.x * 10 + 1),
                       (int)(allObj[objectIndex].position.y * 10),
                       (int)(allObj[objectIndex].position.z * 10 + 1), 0, 0, 0,
                       98);
      ...
      {
        Vector3I itemPosI =
            allObj[entityList[i].locationOnAllObj].actualPosition;
        ...
        for (int j = 0; j < 5; j++) {
          ...
          if (itemPosI.x / 10 == newPposXI && itemPosI.y / 10 == pposYI &&
              itemPosI.z / 10 == newPposZI) {
        ...
        bool isGroundedBlock = checkCollision(
            itemPos.x, itemPos.y + entityList[i].velocityY * deltaTime,
            itemPos.z);
        ...
        if (allObj[entityList[i].locationOnAllObj].sizeT == 1) {
          ...
          for (int j = 0; j < 4; j++) {
            allObj[entityList[i].locationOnAllObj].verticesIPC[j].x =
                voxelVertices4[j].x * 6 + itemPosI.x;
            allObj[entityList[i].locationOnAllObj].verticesIPC[j].y =
                voxelVertices4[j].y * 6 + itemPosI.y;
            allObj[entityList[i].locationOnAllObj].verticesIPC[j].z =
                voxelVertices4[j].z * 6 + itemPosI.z;
          }

          entityList[i].rotation.y += deltaTimeNoSlow * 90;
          Vector3I center = {3 + itemPosI.x, 3 + itemPosI.y, 3 + itemPosI.z};

          addRotateObjectNS(entityList[i].locationOnAllObj,
                            entityList[i].locationOnAllObj, center, 0,
                            entityList[i].rotation.y);
        } else {
          int currentBlockType =
              allBlock[allItem[entityList[i].state].blockId].blockType;

          for (int j = 0; j < allObj[entityList[i].locationOnAllObj].sizeV;
               j++) {
            allObj[entityList[i].locationOnAllObj].verticesIPC[j].x =
                blockTypes[currentBlockType].vertices[0][j].x / 5 + itemPosI.x;
            allObj[entityList[i].locationOnAllObj].verticesIPC[j].y =
                blockTypes[currentBlockType].vertices[0][j].y / 5 + itemPosI.y;
            allObj[entityList[i].locationOnAllObj].verticesIPC[j].z =
                blockTypes[currentBlockType].vertices[0][j].z / 5 + itemPosI.z;
          }
