...
char performanceName[10][35] = {
    "mapUpdate",           "collision, fw, time", "key inputs",
    "vertice calculation", "clear screen",        "triangle rendering",
    "water effect",        "reset Zbuffer",       "make UI",
    "update screen",
};
...
void drawMinecraftFont(int x1, int x2, int y, bool center, int textLength,
                       char text[], color_t textColor,
                       color_t textColorShadow) {
  int xLetterPos = 0;
  int xCenter = 0;
  int totalXLength = 0;
      ...
      for (int x_ = 0; x_ < asciiLength[letterIndex]; x_++) {
        if (x_ < 384) {
          ...
          if ((currentPixel & (1 << (7 - index % 8))) == 0)
            VRAMAddress[(x1 + x_ + xLetterPos + 1 + xCenter) +
                        (y + y_ + 1) * 384] = textColorShadow;
        }
      }
    }
    for (int y_ = 0; y_ < 8; y_++) {
      for (int x_ = 0; x_ < asciiLength[letterIndex]; x_++) {
        if (x_ < 384) {
          ...
          unsigned char currentPixel = asciiMap2[index >> 3];
          if ((currentPixel & (1 << (7 - index % 8))) == 0)
            VRAMAddress[(x1 + x_ + xLetterPos + xCenter) + (y + y_) * 384] =
                textColor;
        }
      }
    }
...
void itemDetails(int posX, int posY, char text[]) {
  ...
  for (int y = posY; y < posY + 16; y++) {
    for (int x = posX; x < posX + totalXLength + 8; x++) {
      if (x < 384)
        VRAMAddress[x + y * 384] =
            combineColors(VRAMAddress[x + y * 384], 0x2104, 60);
    }
  }

  drawMinecraftFont(posX + 4, posX + 4, posY + 4, false, textLength, text,
                    0xdefb, 0x39c7);
}
...
void makeHotbar(int selected) {
  for (int i = 0; i < 9; i++) {
    ...
    if (hotbarBlockTypes[i] != -1) {
      ...
      CopySpriteIcon(&itemIcons2[676 * allItem[hotbarBlockTypes[i]].iconIndex],
                     startPos + 2, 187, 26, 26, 0x0000);

      if (hotbarBlockAmount[i] != 1) {
        char buffer[4];
        sprintf(buffer, "%d", hotbarBlockAmount[i]);

        drawMinecraftFont(startPos + 16, startPos + 16, 202, false,
                          countDigits(hotbarBlockAmount[i]), buffer, 0xdefb,
                          0x39c7);
      }

      if (hotbarBlockHp[i] != allItem[hotbarBlockTypes[i]].breaksIn)
        drawHp(startPos + 4, 210, 2, 22, hotbarBlockHp[i],
               allItem[hotbarBlockTypes[i]].breaksIn);
    }
