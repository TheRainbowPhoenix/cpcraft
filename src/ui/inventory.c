...
void swapItemsChest(int cursorX1, int cursorY1, int cursorX2, int cursorY2) {
  ...
  if (cursorY1 == 6) {
    ...
    typeC1 =
        &allchest[currentChestIndex].chestBlockTypes[cursorY1 * 9 + cursorX1];
    amountC1 =
        &allchest[currentChestIndex].chestBlockAmount[cursorY1 * 9 + cursorX1];
    itemHp1 =
        &allchest[currentChestIndex].chestBlockHp[cursorY1 * 9 + cursorX1];
  }
  ...
  if (cursorY2 == 6) {
    ...
    typeC2 =
        &allchest[currentChestIndex].chestBlockTypes[cursorY2 * 9 + cursorX2];
    amountC2 =
        &allchest[currentChestIndex].chestBlockAmount[cursorY2 * 9 + cursorX2];
    itemHp2 =
        &allchest[currentChestIndex].chestBlockHp[cursorY2 * 9 + cursorX2];
  }
...
void makeUI() {
  ...
  {
    int Buttonlength = 3;
    char text[3][50] = {"Singleplayer", "Options", "Quit"};
    ...
    for (int y = 0; y < 216; y++) {
      for (int x = 0; x < 384; x++) {
        ...
        int yOnTexture = y % 16;
        VRAMAddress[y * 384 + x] =
            textures2[xOnTexture + yOnTexture * 16 + 3 * 256];
      }
    }
    ...
    for (int i = 0; i < Buttonlength; i++) {
      if (cursorY != i) {
        ...
        drawMinecraftFont(112, 272, 106 + i * 30, true, textLength[i], text[i],
                          0xdefb, 0x39c7);
      } else {
  ...
  if (UIState == 10) // world selection
  {
    ...
    if (WorldOpenMode == 0)
      drawMinecraftFont(0, 384, 190, true, 17, "Mode: Create/Load", 0xdefb,
                        0x39c7);
    if (WorldOpenMode == 1)
      drawMinecraftFont(0, 384, 190, true, 12, "Mode: Delete", 0xdefb, 0x39c7);
    if (WorldOpenMode == 2)
      drawMinecraftFont(0, 384, 190, true, 31,
                        "Mode: Convert old/Recover world", 0xdefb, 0x39c7);
    if (WorldOpenMode == 3)
      drawMinecraftFont(0, 384, 190, true, 21, "Mode: Edit world data", 0xdefb,
                        0x39c7);

    drawMinecraftFont(0, 384, 200, true, 27, "Press shift to change modes",
                      0xdefb, 0x39c7);
    ...
    for (int i = 0; i < Buttonlength; i++) {
      ...
      if (cursorY != i) {
        ...
        drawMinecraftFont(112, 272, textHeight[i] + 6, true,
                          textLength[i] + extra, buttonText, 0xdefb, 0x39c7);
      } else {
        CopySprite(buttons[1], 112, textHeight[i], 160, 20, 0x001);
        drawMinecraftFont(112, 272, textHeight[i] + 6, true,
                          textLength[i] + extra, buttonText, 0xfff3, 0x4205);
      }
    }
  }
  if (UIState == 11) // world creation
  {
    int Buttonlength = 8;
    char text[8][50] = {
        "superflat: ",       "Seed: ", "Caves: ",      "Survival: ",
        "Day/night cycle: ", "mobs: ", "Create world", "Cancel",
    };
    int textLength[8] = {11, 6, 7, 10, 17, 6, 12, 6};
    int textHeight[8] = {30, 30, 55, 55, 80, 80, 115, 150};
    Vector2I textWidth[8] = {{0, 191}, {192, 383}, {0, 191}, {192, 383},
                             {0, 191}, {192, 383}, {0, 383}, {0, 383}};
    ...
    for (int i = 0; i < Buttonlength; i++) {
      ...
      if (cursorY != i) {
        ...
        drawMinecraftFont(startX, endX, textHeight[i] + 6, true,
                          textLength[i] + extra, buttonText, 0xdefb, 0x39c7);
      } else {
  ...
  if (UIState == 15) // world conversion
  {
    ...
    drawMinecraftFont(0, 384, 10, true, 22, "Create/Edit world data", 0xdefb,
                      0x39c7);
  ...
  if (UIState == 12) // settings startup menu
  {
    ...
    int textHeight[12] = {30,  55,  55,  80,  80,  105,
                          105, 130, 130, 155, 155, 180};
    Vector2I textWidth[12] = {{0, 383},   {0, 191}, {192, 383}, {0, 191},
                              {192, 383}, {0, 191}, {192, 383}, {0, 191},
                              {192, 383}, {0, 191}, {192, 383}, {0, 383}};
  ...
  if (UIState == 13) // game menu
  {
    ...
    if (keydownlast(KEY_EXE) && !keydownhold(KEY_EXE)) {
      ...
      if (cursorY == 2) {
        saveAllChunk(currentSelectedWorld,
                     worldExists[currentSelectedWorld - 1]);
        worldExists[currentSelectedWorld - 1] = 1;
        worldVersion[currentSelectedWorld - 1] = currentVersion;
    ...
    for (int i = 0; i < Buttonlength; i++) {
      ...
      if (cursorY != i) {
        CopySprite(buttons[0], startX, textHeight[i], 160, 20, 0x001);
        drawMinecraftFont(startX, endX, textHeight[i] + 6, true, textLength[i],
                          text[i], 0xdefb, 0x39c7);
      } else {
        CopySprite(buttons[1], startX, textHeight[i], 160, 20, 0x001);
        drawMinecraftFont(startX, endX, textHeight[i] + 6, true, textLength[i],
                          text[i], 0xfff3, 0x4205);
      }
    }
  }
  ...
  if (UIState == 16) // inventory
  {
    ...
    if (keydownlast(KEY_EXE) && !keydownhold(KEY_EXE)) {
      if (cursorX >= 0) {
        ...
        // addItemToHotbarInventory(craftingRecipes[craftIndex].outputItem+1,
        // craftingRecipes[craftIndex].outputItemAmount);

        if (craftingRecipes[craftIndex].needsCraftingTable == false ||
            isInCraftingTable == true) {
          for (int i = 0; i < totRecourcesNeeded; i++) {
            if (returnTotalItemAmount(
                    craftingRecipes[craftIndex].inputItem[i]) <
                craftingRecipes[craftIndex].inputItemAmount[i])
              continueCraft = 0;
          }

          if (continueCraft == 1) {
            for (int i = 0; i < totRecourcesNeeded; i++)
              removeItemFromInventoryHotbar(
                  craftingRecipes[craftIndex].inputItem[i],
                  craftingRecipes[craftIndex].inputItemAmount[i]);

            addItemToHotbarInventory(
                craftingRecipes[craftIndex].outputItem,
                craftingRecipes[craftIndex].outputItemAmount);
          }
        }
      }
    ...
    CopySpriteNbitMasked(inventory, 108, 42, 268, 132, inventory_palette,
                         0x0001, 4); // crafting menu will be 100 pixels WIDTH
    CopySprite(recipeSelector, 8, 42, 100, 128,
               0x0001); // crafting menu will be 100 pixels WIDTH
    ...
    for (int y = 0; y < 3; y++) {
      for (int x = 0; x < 9; x++) {
          ...
          if (x == cursorX && y == cursorY) {
            for (int y_ = 0; y_ < 26; y_++) {
              for (int x_ = 0; x_ < 26; x_++)
                VRAMAddress[xOnScreen + x_ + 1 + (yOnScreen + y_ + 1) * 384] =
                    0xc638;
            }
          }
        }

        // inventoryBlockTypes[inventoryIndex],
        // inventoryBlockAmount[inventoryIndex]
        ...
        if (itemType != -1) {
          CopySpriteIcon(&itemIcons2[676 * allItem[itemType].iconIndex],
                         xOnScreen + 1, yOnScreen + 1, 26, 26, 0x0000);

          if (itemAmount_ != 1) {
            char buffer[4];
            sprintf(buffer, "%d", itemAmount_);
            drawMinecraftFont(xOnScreen + 16, xOnScreen + 16, yOnScreen + 15,
                              false, countDigits(itemAmount_), buffer, 0xdefb,
                              0x39c7);
          }

          if (itemHp != allItem[itemType].breaksIn && itemHp >= 0 &&
              allItem[itemType].breaksIn != 0)
            drawHp(xOnScreen + 3, yOnScreen + 24, 2, 22, itemHp,
                   allItem[itemType].breaksIn);
        }
      }
    }
    for (int x = 0; x < 9; x++) {
      ...
      for (int x = 0; x < 3; x++) {
        ...
        if (inventoryIndex < craftingRecipeAmount) {
          ...
          if (itemType != -1) {
            ...
            if (isInCraftingTable == false &&
                craftingRecipes[inventoryIndex].needsCraftingTable == true)
              CopySprite(noCraftingTableCross, xOnScreen + 1, yOnScreen + 1, 26,
                         26, 0x0000);
          }
        }
      }
        ...
        if (cursorX < 0) {
          ...
          if (x == cursorXTemp && y == cursorY) {
            ...
            if (itemType < craftingRecipeAmount)
              itemDetails(xOnScreen + 24, yOnScreen + 4,
                          allItem[craftingRecipes[itemType].outputItem].name);
          }
        }
      }
    }
  ...
  if (UIState == 17) // creative menu
  {
    ...
    // CopySprite(inventory, 58, 42, 268, 132, 0x0001);  //crafting menu will be
    // 100 pixels WIDTH
    CopySpriteNbitMasked(inventory, 58, 42, 268, 132, inventory_palette, 0x0001,
                         4); // crafting menu will be 100 pixels WIDTH
  ...
  if (UIState == 18) // inventory chest
  {
    ...
    CopySpriteNbitMasked(chestInv, 58, 0, 268, 216, chestInv_palette, 0x0001,
                         4); // crafting menu will be 100 pixels WIDTH

    int chestIndex = -1;
    for (int i = 0; i < chestAmount; i++) {
      if (allchest[i].used == true) {
        if (removeBlock.x == allchest[i].position.x &&
            removeBlock.y == allchest[i].position.y &&
            removeBlock.z == allchest[i].position.z) {
          chestIndex = i;
          break;
        }
      }
      ...
      for (int y = 0; y < 3; y++) // chest
      {
        for (int x = 0; x < 9; x++) {
          ...
          if (itemType > itemAmount)
            itemType = 0;
          int itemAmount_ =
              allchest[chestIndex].chestBlockAmount[inventoryIndex];
        ...
        for (int x = 0; x < 9; x++) {
          ...
          if (cursorY >= 0 && cursorY < 3) {
            if (x == cursorX && y == cursorY) {
              int inventoryIndex = x + y * 9;
              int itemType =
                  allchest[chestIndex].chestBlockTypes[inventoryIndex];
              if (itemType > itemAmount)
                itemType = 0;
  ...
  if (UIState == 19) // texture pack selector
  {
    ...
    for (int y = 5; y < 185; y++) {
      for (int x = 128; x < 256; x++)
        VRAMAddress[y * 384 + x] =
            combineColors(VRAMAddress[y * 384 + x], 0x0000, 50);
    }
    ...
    for (int i = 0; i < 6; i++) {
      if (i == 5) {
        ...
        if (i < maxTextureIndex) {
          ...
          CopySprite(texturePackIcon[i], 130, 7 + 36 * i, 32, 32,
                     0xffff); // draw icon
          drawMinecraftFont(164, 164, 7 + 36 * i, false,
                            getTextLength(texturePackName[i], 24),
                            texturePackName[i], 0xdefb, 0x39c7); // draw name
          drawMinecraftFont(164, 164, 17 + 36 * i, false,
                            getTextLength(texturePackCreator[i], 24),
                            texturePackCreator[i], 0xdefb,
                            0x39c7); // draw creator

          char versionStr[12] = "";
          memset(versionStr, 0, 12);
          sprintf(versionStr, "V%d", texturePackVersion[i]);
          drawMinecraftFont(164, 164, 27 + 36 * i, false,
                            getTextLength(versionStr, 12), versionStr, 0xdefb,
                            0x39c7); // draw version

          if (currentTexturePackIndex == i)
            drawMinecraftFont(216, 216, 27 + 36 * i, false, 6, "active", 0xdefb,
                              0x39c7); // draw active
        }
        ...
        if (cursorY == i) {
          for (int y = 6 + 36 * i; y < 40 + 36 * i; y++) {
            for (int x = 129; x < 255; x++)
              VRAMAddress[y * 384 + x] =
                  combineColors(VRAMAddress[y * 384 + x], 0x0000, 25);
          }
        }
      }
    }
