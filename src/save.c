...
void replaceOldWater() {
  for (int i = 0; i < TOTALCHUNKWIDTH * TOTALCHUNKWIDTH; i++) {
    ...
    loadingScreen(i, TOTALCHUNKWIDTH * TOTALCHUNKWIDTH,
                  "Convering old water...", 22);
  }
}
...
void makeStartFolder() {
  ...
  Bfile_StrToName_ncpy(pDir1, (unsigned char *)dirLocation1,
                       sizeof(dirLocation1));
...
void saveChestData(int world) {
  ...
  for (int chestI = 0; chestI < chestAmount; chestI++) {
    ...
    for (int k = 0; k < 27; k++) {
      for (int j = 0; j < 2; j++)
        chestData_[i + 13 + j + (k * 2)] =
            (allchest[chestI].chestBlockTypes[k] >> (j * 8)) & 0xFF;
    }
    for (int k = 0; k < 27; k++) {
      for (int j = 0; j < 2; j++)
        chestData_[i + 67 + j + (k * 2)] =
            (allchest[chestI].chestBlockAmount[k] >> (j * 8)) & 0xFF;
    }
    for (int k = 0; k < 27; k++) {
      for (int j = 0; j < 2; j++)
        chestData_[i + 121 + j + (k * 2)] =
            (allchest[chestI].chestBlockHp[k] >> (j * 8)) & 0xFF;
    }
  }
...
void loadChestData(int world) {
  ...
  if (hFile > 0) {
    ...
    for (int chestI = 0; chestI < chestAmount; chestI++) {
      ...
      for (int j = 0; j < 4; j++)
        allchest[chestI].position.x |=
            ((chestData_[i + 1 + j] & 0xff) << j * 8);
      for (int j = 0; j < 4; j++)
        allchest[chestI].position.y |=
            ((chestData_[i + 5 + j] & 0xff) << j * 8);
      for (int j = 0; j < 4; j++)
        allchest[chestI].position.z |=
            ((chestData_[i + 9 + j] & 0xff) << j * 8);

      for (int k = 0; k < 27; k++) {
        allchest[chestI].chestBlockTypes[k] = 0;

        for (int j = 0; j < 2; j++)
          allchest[chestI].chestBlockTypes[k] |=
              ((chestData_[i + 13 + j + (k * 2)] & 0xff) << j * 8);
      }
      for (int k = 0; k < 27; k++) {
        allchest[chestI].chestBlockAmount[k] = 0;

        for (int j = 0; j < 2; j++)
          allchest[chestI].chestBlockAmount[k] |=
              ((chestData_[i + 67 + j + (k * 2)] & 0xff) << j * 8);
      }
      for (int k = 0; k < 27; k++) {
        allchest[chestI].chestBlockHp[k] = 0;

        for (int j = 0; j < 2; j++)
          allchest[chestI].chestBlockHp[k] |=
              ((chestData_[i + 121 + j + (k * 2)] & 0xff) << j * 8);
      }
    }
  } else
    saveChestData(world);
...
void saveChunkExtraData(int index, int world, bool exists) {
  ...
  int hFile = Bfile_OpenFile_OS(
      pFile, 3, NULL); // Get handle          //0=read, 1=read_share, 2=write,
                       // 3=readwrite, 4=readwriteshare
  Bfile_WriteFile_OS(hFile, &blockData[index * WIDTH * WIDTH * HEIGHT],
                     sizeOfFile);
  Bfile_CloseFile_OS(hFile);
...
void loadChunkExtraData(int index, int world) {
  ...
  Bfile_ReadFile_OS(hFile, &blockData[index * WIDTH * WIDTH * HEIGHT], length,
                    0);
  Bfile_CloseFile_OS(hFile);
...
void saveCompressedChunk(int index, int world, bool exists) {
  ...
  memcpy(data + WIDTH * WIDTH * HEIGHT,
         &blockData[index * WIDTH * WIDTH * HEIGHT], WIDTH * WIDTH * HEIGHT);
...
void loadCompressedChunk(int index, int world) {
  ...
  memcpy(&blockData[index * WIDTH * WIDTH * HEIGHT],
         decompressedData + WIDTH * WIDTH * HEIGHT, WIDTH * WIDTH * HEIGHT);
...
void loadChestDataCompressed(int world) {
  ...
  if (hFile > 0) {
    ...
    for (int chestI = 0; chestI < chestAmount; chestI++) {
      ...
      for (int j = 0; j < 4; j++)
        allchest[chestI].position.x |=
            ((decompressedData[i + 1 + j] & 0xff) << j * 8);
      for (int j = 0; j < 4; j++)
        allchest[chestI].position.y |=
            ((decompressedData[i + 5 + j] & 0xff) << j * 8);
      for (int j = 0; j < 4; j++)
        allchest[chestI].position.z |=
            ((decompressedData[i + 9 + j] & 0xff) << j * 8);
      ...
      for (int k = 0; k < 27; k++) {
        ...
        for (int j = 0; j < 2; j++)
          allchest[chestI].chestBlockTypes[k] |=
              ((decompressedData[i + 13 + j + (k * 2)] & 0xff) << j * 8);
      }
      for (int k = 0; k < 27; k++) {
        allchest[chestI].chestBlockAmount[k] = 0;
        ...
        for (int j = 0; j < 2; j++)
          allchest[chestI].chestBlockAmount[k] |=
              ((decompressedData[i + 67 + j + (k * 2)] & 0xff) << j * 8);
      }
    }
  } else
    saveChestData(world);
...
void deleteOldWorld(int world) {
  ...
  for (int i = 0; i < TOTALCHUNKWIDTH * TOTALCHUNKWIDTH; i++) {
    ...
    // getting entry for chunkFile"\\\\fls0\\fxcraft\\world%d\\chunkED%d",
    // world, i
    unsigned short pFile[sizeof(fileLocation) * 2]; // Make buffer
    Bfile_StrToName_ncpy(pFile, (unsigned char *)fileLocation,
                         sizeof(fileLocation));
  ...
  //	unsigned short pFile[sizeof(fileLocation)*2]; // Make buffer
  //	Bfile_StrToName_ncpy(pFile, (unsigned char*)fileLocation,
  // sizeof(fileLocation)); 	Bfile_DeleteEntry(pFile);
...
void savePlayerData(int world) {
  ...
  for (int i = 0; i < 27; i++) {
    ...
    for (int j = 2; j < 4; j++)
      playerData[74 + i * 6 + j] =
          (inventoryBlockAmount[i] >> (j - 2) * 8) & 0xFF;
    for (int j = 4; j < 6; j++)
...
void loadPlayerData(int world) {
  ...
  if (hFile > 0) {
    ...
    for (int i = 0; i < 9; i++) {
      ...
      for (int j = 2; j < 4; j++)
        hotbarBlockAmount[i] |=
            ((playerData[20 + i * 6 + j] & 0xFF) << (j - 2) * 8);
      for (int j = 4; j < 6; j++)
        hotbarBlockHp[i] |=
            ((playerData[20 + i * 6 + j] & 0xFF) << (j - 4) * 8);
    }
    ...
    for (int i = 0; i < 27; i++) {
      ...
      for (int j = 0; j < 2; j++)
        inventoryBlockTypes[i] |=
            ((playerData[74 + i * 6 + j] & 0xFF) << j * 8);
      for (int j = 2; j < 4; j++)
        inventoryBlockAmount[i] |=
            ((playerData[74 + i * 6 + j] & 0xFF) << (j - 2) * 8);
      for (int j = 4; j < 6; j++)
        inventoryBlockHp[i] |=
            ((playerData[74 + i * 6 + j] & 0xFF) << (j - 4) * 8);
    }
  }
}
void loadPlayerDataV4(int world) {
  ...
  if (hFile > 0) {
    ...
    for (int i = 0; i < 9; i++) {
      ...
      for (int j = 2; j < 4; j++)
        hotbarBlockAmount[i] |=
            ((playerData[20 + i * 4 + j] & 0xFF) << (j - 2) * 8);
    }
    ...
    for (int i = 0; i < 27; i++) {
      ...
      for (int j = 0; j < 2; j++)
        inventoryBlockTypes[i] |=
            ((playerData[56 + i * 4 + j] & 0xFF) << j * 8);
      for (int j = 2; j < 4; j++)
        inventoryBlockAmount[i] |=
            ((playerData[56 + i * 4 + j] & 0xFF) << (j - 2) * 8);
    }
  }
}
void saveAllChunk(int world, bool exists) {
  ...
  if (exists == false) // cleating dir
  {
    ...
    Bfile_StrToName_ncpy(pDir, (unsigned char *)dirLocation2,
                         sizeof(dirLocation2));
  ...
  if (worldVersion[world - 1] <= currentVersion) {
    ...
    for (int i = 0; i < TOTALCHUNKWIDTH * TOTALCHUNKWIDTH; i++) {
      if (worldVersion[world - 1] < 4) {
        ...
        if (worldVersion[world - 1] >=
            2) // only load if its added as a new version
    ...
    if (worldVersion[world - 1] < 5) // save the health of tools
    {
      ...
      Bfile_StrToName_ncpy(pFile1, (unsigned char *)fileLocation1,
                           sizeof(fileLocation1));
      ...
      Bfile_StrToName_ncpy(pFile2, (unsigned char *)fileLocation2,
