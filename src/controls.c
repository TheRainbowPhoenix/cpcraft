...
static int get_key_index(int basic_keycode) { if (basic_keycode < 0) return 0; return basic_keycode % KEY_STATE_SIZE; }
void keyupdate(void) {
    ...
    for (size_t i = 0; i < sizeof(keys_to_track)/sizeof(keys_to_track[0]); i++) {
        ...
        if (sc != 0) key_states[get_key_index(k)] = Input_GetKeyState(sc);
    }
}
int keydownlast(int basic_keycode) { return key_states[get_key_index(basic_keycode)]; }
int keydownhold(int basic_keycode) { return last_key_states[get_key_index(basic_keycode)]; }
