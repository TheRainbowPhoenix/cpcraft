    ...
    if (UIState < 9 || UIState > 12) {
      ...
      for (int chunkX = 0; chunkX < TOTALCHUNKWIDTH; chunkX++) {
        for (int chunkZ = 0; chunkZ < TOTALCHUNKWIDTH; chunkZ++) {
          if ((abs_int(chunkX - (int)(PPosX / WIDTH)) +
               abs_int(chunkZ - (int)(PPosZ / WIDTH))) <=
      ...
      if (chunksToUpdate > 0) {
        while (redstoneUpdateTimer1 >
               (float)((0.25 / (float)chunksToUpdate) *
                       ((float)currentRedstoneChunk + 1))) {
      ...
      if (time > 1) {
        ...
        if (dayNightCycle == true) {
          ...
          if ((dayTime > 1140 && dayTime < 1200) ||
              (dayTime > 0 && dayTime < 80)) {
            dayTimeChange++;
            if (dayTimeChange == 10) {
              dayTimeChange = 0;
              skyBrightness++;
      ...
      bool isInWater = checkIfInWater(
          PPosX + forward.x * (playerMovementSpeed * deltaTime), PPosY - 1.4,
      ...
      if (survival == true) {
        ...
        isGrounded =
            checkCollision(PPosX, PPosY - 1.8 + velocityY * deltaTime, PPosZ);
      ...
      if (UIState == 0) {
        ...
        if (keydownlast(KEY_PRGM_8) && keydownhold(KEY_PRGM_8)) {
          if (survival == false) {
            ...
            if (checkCollision(PPosX + forward.x *
                                           (playerMovementSpeed * deltaTime),
                               PPosY - 1.2, PPosZ) == false)
              PPosX += forward.x * (playerMovementSpeed * deltaTime);
            if (checkCollision(PPosX, PPosY - 1.2,
                               PPosZ - forward.z * (playerMovementSpeed *
                                                    deltaTime)) == false)
              PPosZ -= forward.z * (playerMovementSpeed * deltaTime);
          }
        }
        ...
        if (keydownlast(KEY_PRGM_6) && keydownhold(KEY_PRGM_6)) {
          if (survival == false) {
            ...
            if (checkCollision(PPosX +
                                   right.x * (playerMovementSpeed * deltaTime),
                               PPosY - 1.2, PPosZ) == false)
              PPosX += right.x * (playerMovementSpeed * deltaTime);
            if (checkCollision(PPosX, PPosY - 1.2,
                               PPosZ - right.z * (playerMovementSpeed *
                                                  deltaTime)) == false)
              PPosZ -= right.z * (playerMovementSpeed * deltaTime);
          }
        }
        ...
        if (survival == true) {
          if (keydownlast(KEY_PRGM_7) && keydownhold(KEY_PRGM_7)) {
              ...
              if (entityList[entityInFrontIndex].type != 3) {
                if (justPressedHitButton == false) {
                  ...
                  if (entityList[entityInFrontIndex].type == 1) // sheep
                  {
                    ...
                    int objectIndex =
                        entityList[entityInFrontIndex].locationOnAllObj;
                    int objectIndex2 =
                        entityList[entityList[entityInFrontIndex].extraEntity]
                            .locationOnAllObj;

                    for (int i = 0; i < allObj[objectIndex].sizeT; i++)
                      allObj[objectIndex].solidColor[i] = combineColors(
                          allObj[objectIndex].solidColor[i], 0xfa8a, 50);

                    for (int i = 0; i < allObj[objectIndex2].sizeT; i++)
                      allObj[objectIndex2].solidColor[i] = combineColors(
                          allObj[objectIndex2].solidColor[i], 0xfa8a, 50);
            ...
            if (hasEntityInFront == false) {
              ...
              int currentChunk = (removeBlock.x / WIDTH) +
                                 (removeBlock.z / WIDTH) * TOTALCHUNKWIDTH;
              int blockIndex = (removeBlock.x % WIDTH) +
                               (removeBlock.z % WIDTH) * WIDTH +
                               (removeBlock.y) * WIDTH * WIDTH;

              if (destructionBlockIndex !=
                  blockIndex) // reset if looking at diffrent block
              {
              ...
              if (currentToolType == allBlock[blockType].toolToBeak)
                totalDestructionTimer =
                    allBlock[blockType].destroyTime[currentToolLevel];
              else
      ...
      cameraRotationX =
          ToRadians(rotationX); // recalculate because we just moved the camera
        ...
        if (allObj[obj].active == true) {
          for (int allVert = 0; allVert < allObj[obj].sizeV; allVert++) {
            ...
            if (z > 1 && z < 65000) {
              ...
              if (xOnScreen < 4096 && xOnScreen > -4096 && yOnScreen < 4096 &&
                  yOnScreen > -4096) {
      ...
      if ((renderingMode == 2) || (renderingMode != 2 && clearScreen == true)) {
        if (beautifulSky == false) {
          dmaStartFill((int)screenColor, (int)(GetVRAMAddress()) & 0x1FFFFFFF,
                       165888);
          DmaWaitNextC0();
        } else
          makeSkyBox();
      ...
      for (int obj = 0; obj < 64; obj++) {
        if (allObj[obj].active == true) {
          for (int alltri = 0; alltri < allObj[obj].sizeT; alltri++) {
            ...
            if (v1.z != 65001 && v2.z != 65001 && v3.z != 65001 &&
                v4.z != 65001) {
              if ((v1.x > 0 && v1.x < resX) || (v2.x > 0 && v2.x < resX) ||
                  (v3.x > 0 && v3.x < resX) || (v4.x > 0 && v4.x < resX)) {
                if ((v1.y > 0 && v1.y < resY) || (v2.y > 0 && v2.y < resY) ||
                    (v3.y > 0 && v3.y < resY) || (v4.y > 0 && v4.y < resY)) {
                  if (renderTextured == false) {
                    if ((allObj[obj].color[alltri] == 12 ||
                         allObj[obj].color[alltri] == 13) &&
                        renderUnderwater == true) {
                    ...
                    if (textureType[allObj[obj].color[alltri]] == 2 &&
                        renderUnderwater == true) {
                      waterQuads[currentWaterQuadLocation] = alltri;
                      waterQuadsObj[currentWaterQuadLocation] = obj;
                      currentWaterQuadLocation++;
                  ...
                  Vector3I normal = {allObj[obj].normal[alltri].x,
                                     allObj[obj].normal[alltri].y,
                                     allObj[obj].normal[alltri].z};
                      ...
                      if (renderTextured == false) {
                        if (pixelSize == 2) {
                          if (brightness != 16) {
                            renderTrianglePX2(
                                &V1_, &V2_, &V3_,
                                allColors[allObj[obj].color[alltri]], Depth,
                                brightness);
                            renderTrianglePX2(
                                &V1_, &V3_, &V4_,
                                allColors[allObj[obj].color[alltri]], Depth,
                          ...
                          if (brightness != 16) {
                            renderTriangle(&V1_, &V2_, &V3_,
                                           allColors[allObj[obj].color[alltri]],
                                           Depth, brightness);
                            renderTriangle(&V1_, &V3_, &V4_,
                                           allColors[allObj[obj].color[alltri]],
                                           Depth, brightness);
                          } else {
                            renderTriangleNB(
                        ...
                        int texY =
                            (allObj[obj].textureSize[alltri] >> 4) & 0x0F;
                        ...
                        if (pixelSize == 2) {
                          if (brightness != 16) {
                            renderTriangleTexturedPX2T(
                                V1_, V2_, V3_, AT, BT, CT, Depth, texX, texY,
                                &textures2[allObj[obj].color[alltri] * 256],
                                brightness);
                            renderTriangleTexturedPX2T(
                                V1_, V3_, V4_, AT, CT, DT, Depth, texX, texY,
                                &textures2[allObj[obj].color[alltri] * 256],
                                brightness);
                          } else {
                            renderTriangleTexturedNBPX2T(
                                V1_, V2_, V3_, AT, BT, CT, Depth, texX, texY,
                                &textures2[allObj[obj].color[alltri] * 256]);
                            renderTriangleTexturedNBPX2T(
                                V1_, V3_, V4_, AT, CT, DT, Depth, texX, texY,
                ...
                if ((v1.y > 0 && v1.y < resY) || (v2.y > 0 && v2.y < resY) ||
                  ...
                  if (renderingMode == 0 || renderingMode == 2) // normal mode
                  {
                    ...
                    if (lighting == true) {
                      ...
                      int chunkIndex =
                          (pos.x / WIDTH) + (pos.z / WIDTH) * TOTALCHUNKWIDTH;
                    ...
                    if (entityList[obj - 100].renderTextured == false) {
                      if (pixelSize == 2) {
                        if (brightness != 16) {
                          renderTrianglePX2(&V1_, &V2_, &V3_,
                                            allObj[obj].solidColor[alltri],
                                            Depth, brightness);
                          renderTrianglePX2(&V1_, &V3_, &V4_,
                                            allObj[obj].solidColor[alltri],
                                            Depth, brightness);
                        } else {
                          renderTriangleNBPX2(&V1_, &V2_, &V3_,
                                              allObj[obj].solidColor[alltri],
                                              Depth);
                          renderTriangleNBPX2(&V1_, &V3_, &V4_,
                                              allObj[obj].solidColor[alltri],
                                              Depth);
                        }
                      } else {
                        if (brightness != 16) {
                          renderTriangle(&V1_, &V2_, &V3_,
                                         allObj[obj].solidColor[alltri], Depth,
                                         brightness);
                          renderTriangle(&V1_, &V3_, &V4_,
                                         allObj[obj].solidColor[alltri], Depth,
                                         brightness);
                        } else {
                          renderTriangleNB(&V1_, &V2_, &V3_,
                                           allObj[obj].solidColor[alltri],
                                           Depth);
                          renderTriangleNB(&V1_, &V3_, &V4_,
                                           allObj[obj].solidColor[alltri],
                                           Depth);
                        }
                      }
                      ...
                      if (pixelSize == 2) {
                        if (brightness != 16) {
                          renderTriangleTexturedBlackTransPX2T(
                              V1_, V2_, V3_, AT, BT, CT, Depth, texX, texY,
                              &textures2[allObj[obj].color[alltri] * 256],
                              brightness);
                          renderTriangleTexturedBlackTransPX2T(
                          ...
                          renderTriangleTexturedBlackTransNBPX2T(
                              V1_, V2_, V3_, AT, BT, CT, Depth, texX, texY,
                              &textures2[allObj[obj].color[alltri] * 256]);
                          renderTriangleTexturedBlackTransNBPX2T(
                              V1_, V3_, V4_, AT, CT, DT, Depth, texX, texY,
                        ...
                        if (brightness != 16) {
                          renderTriangleTexturedBlackTransT(
                              V1_, V2_, V3_, AT, BT, CT, Depth, texX, texY,
                              &textures2[allObj[obj].color[alltri] * 256],
                              brightness);
                          renderTriangleTexturedBlackTransT(
                          ...
                          renderTriangleTexturedBlackTransNBT(
                              V1_, V2_, V3_, AT, BT, CT, Depth, texX, texY,
                              &textures2[allObj[obj].color[alltri] * 256]);
                          renderTriangleTexturedBlackTransNBT(
                              V1_, V3_, V4_, AT, CT, DT, Depth, texX, texY,
        ...
        if (renderingMode == 0 || renderingMode == 2) // normal mode
        {
          ...
          if (pixelSize == 2) {
            if (brightness != 16) {
              renderTriangleTexturedBlackTransPX2T(
                  V1_, V2_, V3_, AT, BT, CT, Depth, texX, texY,
                  &textures2[allObj[obj].color[alltri] * 256], brightness);
              renderTriangleTexturedBlackTransPX2T(
                  V1_, V3_, V4_, AT, CT, DT, Depth, texX, texY,
                  &textures2[allObj[obj].color[alltri] * 256], brightness);
            } else {
      ...
      for (int i = 0; i < currentWaterQuadLocation; i++) {
        ...
        if (renderingMode == 0 || renderingMode == 2) // normal mode
        {
          ...
          if (renderTextured == false) {
            if (pixelSize == 2) {
              if (brightness != 16) {
                renderTrianglePX2Trans(&V1_, &V2_, &V3_,
                                       allColors[allObj[obj].color[alltri]],
                                       Depth, brightness);
                renderTrianglePX2Trans(&V1_, &V3_, &V4_,
                                       allColors[allObj[obj].color[alltri]],
                                       Depth, brightness);
              } else {
                renderTriangleNBPX2Trans(&V1_, &V2_, &V3_,
                                         allColors[allObj[obj].color[alltri]],
                                         Depth);
                renderTriangleNBPX2Trans(&V1_, &V3_, &V4_,
                                         allColors[allObj[obj].color[alltri]],
                                         Depth);
              }
            } else {
              if (brightness != 16) {
                renderTriangleTrans(&V1_, &V2_, &V3_,
                                    allColors[allObj[obj].color[alltri]], Depth,
                                    brightness);
                renderTriangleTrans(&V1_, &V3_, &V4_,
                                    allColors[allObj[obj].color[alltri]], Depth,
                                    brightness);
              } else {
                renderTriangleNBTrans(&V1_, &V2_, &V3_,
                                      allColors[allObj[obj].color[alltri]],
                                      Depth);
                renderTriangleNBTrans(&V1_, &V3_, &V4_,
                                      allColors[allObj[obj].color[alltri]],
                                      Depth);
              }
            }
            ...
            if (pixelSize == 2) {
              if (brightness != 16) {
                renderTriangleTexturedPX2TransT(
                    V1_, V2_, V3_, AT, BT, CT, Depth, texX, texY,
                    &textures2[allObj[obj].color[alltri] * 256], brightness);
                renderTriangleTexturedPX2TransT(
                    V1_, V3_, V4_, AT, CT, DT, Depth, texX, texY,
                ...
                renderTriangleTexturedNBPX2TransT(
                    V1_, V2_, V3_, AT, BT, CT, Depth, texX, texY,
                    &textures2[allObj[obj].color[alltri] * 256]);
                renderTriangleTexturedNBPX2TransT(
                    V1_, V3_, V4_, AT, CT, DT, Depth, texX, texY,
              ...
              if (brightness != 16) {
                ...
                renderTriangleTexturedNBTransT(
                    V1_, V2_, V3_, AT, BT, CT, Depth, texX, texY,
                    &textures2[allObj[obj].color[alltri] * 256]);
                renderTriangleTexturedNBTransT(
                    V1_, V3_, V4_, AT, CT, DT, Depth, texX, texY,
      ...
      for (int allparti = 0; allparti < partiLength; allparti++) {
        ...
        if (currentParti.used == true) {
          ...
          if (z > 10 && z < 65000) {
            ...
            if (xOnScreen < 32768 && xOnScreen > -32768 && yOnScreen < 32768 &&
                yOnScreen > -32768) {
              sp.x = xOnScreen;
              sp.y = yOnScreen;
              sp.z = z;
              ...
              if (allparticles[allparti].time <
                  allparticles[allparti].maxTime) {
                renderParticle(10, currentParti.mode, currentParti.color, sp,
                               10);

                if (currentParti.gravity == true)
                  allparticles[allparti].velocity.y -= 9.81f * deltaTimeNoSlow;

                allparticles[allparti].position.x +=
                    allparticles[allparti].velocity.x * deltaTime;
                allparticles[allparti].position.y +=
                    allparticles[allparti].velocity.y * deltaTime;
                allparticles[allparti].position.z +=
                    allparticles[allparti].velocity.z * deltaTime;
              } else
