  ...
  if (removeBlock.x != -1) {
    ...
    if (blockType == 20) // cook item in furnace
    {
      ...
      if (con != -1) {
        ...
        makeItem((int)(removeBlock.x * 10) + 4, (int)(removeBlock.y * 10) + 14,
                 (int)(removeBlock.z * 10) + 4, 0, 0, 0,
                 furnaceRecipes[con].outputItem);
      }
      return;
    }
    ...
    if (allBlock[blockType].blockType == 18) // botton presses
    {
      if (blockType == 64)
        blockData[chunkIndex * WIDTH * WIDTH * HEIGHT + blockIndex] =
            ((blockData[chunkIndex * WIDTH * WIDTH * HEIGHT + blockIndex] &
              0b11001110) |
             0b00110001); // leave on for 4 redstone ticks
      if (blockType == 65)
        blockData[chunkIndex * WIDTH * WIDTH * HEIGHT + blockIndex] =
            ((blockData[chunkIndex * WIDTH * WIDTH * HEIGHT + blockIndex] &
              0b11001110) |
             0b00100001); // leave on for 3 redstone ticks
      ...
      if (((blockData[chunkIndex * WIDTH * WIDTH * HEIGHT + blockIndex] &
        ...
        blockData[chunkIndex * WIDTH * WIDTH * HEIGHT + blockIndex +
            ...
            ((blockData[chunkIndex * WIDTH * WIDTH * HEIGHT + blockIndex +
              ...
              0b11111011) |
             0b00000100);
      } else if (((blockData[chunkIndex * WIDTH * WIDTH * HEIGHT + blockIndex] &
                   0b00000100) >>
                  2) == 1) {
        if ((blockData[chunkIndex * WIDTH * WIDTH * HEIGHT + blockIndex] &
             0b00000011) == 0)
      ...
      if (entityInFrontIndex > -1) {
        if (entityList[entityInFrontIndex].type == 1) {
          if (entityList[entityInFrontIndex].hasFur == true) {
            ...
            allObj[entityList[entityList[entityInFrontIndex].extraEntity]
                       .locationOnAllObj]
                .active = false;
            makeItem(
                (int)(allObj[entityList[entityInFrontIndex].locationOnAllObj]
                          .position.x *
                      10),
                (int)(allObj[entityList[entityInFrontIndex].locationOnAllObj]
                          .position.y *
    ...
    if (blockSelected == 66) // feed a feedable mob
    {
      if (entityInFrontIndex > -1) {
        if (entityList[entityInFrontIndex].type == 1 ||
            entityList[entityInFrontIndex].type == 2) {
    ...
    if (allItem[blockSelected].toolType == 0 && blockSelected != -1 &&
        hotbarBlockAmount[currentSlot] > 0) {
      if (blockSelected == 65) {
