    ...
    if(blocksRenderd[chunkIndex] == false)
    {
                    ...
                    for (int y = 0; y < height; y++)
                    {
                        if(y <= maxY)
                        {
                            if(y == maxY)
                            {
                                ...
                                if(mobs == true)
                                {
                                    if(generate_random2D(SEED, 1024, x+chunkX*width, z+chunkY*width) == 2 && maxY > 18+terrainHeight)
                                    makePig(x+chunkX*width, y+1, z+chunkY*width);
                                    if(generate_random2D(SEED, 1024, x+chunkX*width, z+chunkY*width) == 3 && maxY > 18+terrainHeight)
                                    makeSheep2(x+chunkX*width, y+1, z+chunkY*width);
                                }

                                if(generate_random2D(SEED, 128, x+chunkX*width, z+chunkY*width) == 1 && maxY > 20+terrainHeight)
        ...
        if(advancedTerrain == 1)
        {
            ...
            for (int x = 0; x < width; x++)
            {
                for (int z = 0; z < width; z++)
                {
                    int maxY = perlin2d(x + chunkX * width, z + chunkY * width, 0.05f, 1) * 8 + 24;
                    ...
                    for (int y = 0; y < height; y++)
                    {
                        if(y <= maxY)
                        {
                            if(y == maxY)
                            {
                                ...
                                if(generate_random2D(SEED, 256, x+chunkX*width, z+chunkY*width) == 0 && maxY > 18)
                                generateTree(x+chunkX*width, y+1, z+chunkY*width, generate_random2D(SEED, 4, x+chunkX*width, z+chunkY*width) + 4);
                            }
                            if(y == maxY-1 || y == maxY-2)
...
{
    if(chunkX >= 0 && chunkX <= totalChunkWidth && chunkZ >= 0 && chunkZ <= totalChunkWidth)
    {
                ...
                for (int z = 0; z < width; z++)
                {
                    ...
                    if(currentBlockIndex != 0)
                    {
                        for (int i = 0; i < blockTypes[currentBlockType].facesLength; i++)
                        {
                            ...
                            {
                                int x2 = chunkX12+x+blockTypes[currentBlockType].dir[blockRotation][i].x;
                                int y2 = y+blockTypes[currentBlockType].dir[blockRotation][i].y;
                                int z2 = chunkZ12+z+blockTypes[currentBlockType].dir[blockRotation][i].z;

                                int chunkX2 = x2/width;
                                int chunkZ2 = z2/width;

                                if(chunkX2 >= 0 && chunkX2 < totalChunkWidth && chunkZ2 >= 0 && chunkZ2 < totalChunkWidth)
                                {
                                    if(y2 >= 0 && y2 < height)
                                    {
                                        ...
                                        int blockIndex2 = xInChunk2 + zInChunk2 * width + y2 * width * width;

                                        int currentBlockIndex2 = blocks[chunkIndex2][blockIndex2];
                                        int currentBlockType2 = allBlock[currentBlockIndex2].blockType;

                                        int blockIndexR2 = blockIndex2 + chunkIndex2 * width * width * height;
        ...
        allObj[chunkIndex].triangles   = tlsf_malloc(tlsf, sizeof(Vector4S) * totalTriangleLength     );
        allObj[chunkIndex].normal      = tlsf_malloc(tlsf, sizeof(Vector3B) * totalTriangleLength     );
        allObj[chunkIndex].color       = tlsf_malloc(tlsf, sizeof(unsigned char) * totalTriangleLength);
        allObj[chunkIndex].brightnes   = tlsf_malloc(tlsf, sizeof(unsigned char) * totalTriangleLength);
        allObj[chunkIndex].textureSize = tlsf_malloc(tlsf, sizeof(unsigned char) * totalTriangleLength);
                            ...
                            if(blockTypes[currentBlockType].doSideCheck[i] == 1)
                            {
                                ...
                                if(chunkX2 >= 0 && chunkX2 < totalChunkWidth && chunkZ2 >= 0 && chunkZ2 < totalChunkWidth)
                                {
                                    if(y2 >= 0 && y2 < height)
                                    {
                                        ...
                                        if(blockTypes[currentBlockType2].otherBlocksRenderFace[blockRotation2][blockSideIndex] == 0 || (blockTypes[currentBlockType2].isTransprent == true && blockTypes[currentBlockType].isTransprent == false) || (blockTypes[currentBlockType2].isfluid == true && blockTypes[currentBlockType].isfluid == false))
                                        {
                                            allObj[chunkIndex].triangles[currentTriangleLength].x = currentVerticeLength+0;
                                            allObj[chunkIndex].triangles[currentTriangleLength].y = currentVerticeLength+1;
                                            allObj[chunkIndex].triangles[currentTriangleLength].z = currentVerticeLength+2;
                                            ...
                                            allObj[chunkIndex].color[currentTriangleLength] = allBlock[blocks[chunkIndex][blockIndex]].noTextureColorIndex[(x + y + z) % 2][i];
                                            ...
                                            {
                                                int lightIndex = blockIndex2 + chunkIndex2 * width * width * height;
                                            ...
                                            allObj[chunkIndex].brightnes[currentTriangleLength] = brightness;

                                            verticesIPCTemp[currentVerticeLength+0].x = (x + chunkX * width) * 10 + blockTypes[currentBlockType].vertices[blockRotation][blockTypes[currentBlockType].faces[i].x].x;
                                            verticesIPCTemp[currentVerticeLength+0].y = y * 10 +                    blockTypes[currentBlockType].vertices[blockRotation][blockTypes[currentBlockType].faces[i].x].y;
                                            verticesIPCTemp[currentVerticeLength+0].z = (z + chunkZ * width) * 10 + blockTypes[currentBlockType].vertices[blockRotation][blockTypes[currentBlockType].faces[i].x].z;

                                            verticesIPCTemp[currentVerticeLength+1].x = (x + chunkX * width) * 10 + blockTypes[currentBlockType].vertices[blockRotation][blockTypes[currentBlockType].faces[i].y].x;
                                            verticesIPCTemp[currentVerticeLength+1].y = y * 10 +                    blockTypes[currentBlockType].vertices[blockRotation][blockTypes[currentBlockType].faces[i].y].y;
                                            verticesIPCTemp[currentVerticeLength+1].z = (z + chunkZ * width) * 10 + blockTypes[currentBlockType].vertices[blockRotation][blockTypes[currentBlockType].faces[i].y].z;

                                            verticesIPCTemp[currentVerticeLength+2].x = (x + chunkX * width) * 10 + blockTypes[currentBlockType].vertices[blockRotation][blockTypes[currentBlockType].faces[i].z].x;
                                            verticesIPCTemp[currentVerticeLength+2].y = y * 10 +                    blockTypes[currentBlockType].vertices[blockRotation][blockTypes[currentBlockType].faces[i].z].y;
                                            verticesIPCTemp[currentVerticeLength+2].z = (z + chunkZ * width) * 10 + blockTypes[currentBlockType].vertices[blockRotation][blockTypes[currentBlockType].faces[i].z].z;

                                            verticesIPCTemp[currentVerticeLength+3].x = (x + chunkX * width) * 10 + blockTypes[currentBlockType].vertices[blockRotation][blockTypes[currentBlockType].faces[i].w].x;
                                            verticesIPCTemp[currentVerticeLength+3].y = y * 10 +                    blockTypes[currentBlockType].vertices[blockRotation][blockTypes[currentBlockType].faces[i].w].y;
                                            verticesIPCTemp[currentVerticeLength+3].z = (z + chunkZ * width) * 10 + blockTypes[currentBlockType].vertices[blockRotation][blockTypes[currentBlockType].faces[i].w].z;
        ...
        if(compressVertices == true)
        {
            ...
            for (int i = 0; i < totalVerticeLength; i++)
            {
                ...
                for (int j = 0; j < totalVerticeLength; j++)
                {
                    ...
                    if(positionTable[j].x == verticesIPCTemp[i].x && positionTable[j].y == verticesIPCTemp[i].y && positionTable[j].z == verticesIPCTemp[i].z)
                    {
                        oldToNew[i] = j;
                        makeNewIndex = false;
                        break;
            ...
            allObj[chunkIndex].verticesIPC = tlsf_malloc(tlsf, sizeof(Vector3S) * newVerticeLength);
            ...
            {
                allObj[chunkIndex].triangles[i].x = oldToNew[allObj[chunkIndex].triangles[i].x];
                allObj[chunkIndex].triangles[i].y = oldToNew[allObj[chunkIndex].triangles[i].y];
                allObj[chunkIndex].triangles[i].z = oldToNew[allObj[chunkIndex].triangles[i].z];
                allObj[chunkIndex].triangles[i].w = oldToNew[allObj[chunkIndex].triangles[i].w];
            }
        ...
        {
            allObj[chunkIndex].verticesIPC   = tlsf_malloc(tlsf, sizeof(Vector3S) * totalVerticeLength     );
