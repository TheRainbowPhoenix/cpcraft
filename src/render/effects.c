...
void renderBlockDestruction(float timeDone, float totalTime) {
  ...
  Vector3B directions[6] = {{0, 1, 0},  {0, 0, -1}, {-1, 0, 0},
                            {0, -1, 0}, {1, 0, 0},  {0, 0, 1}};
  Vector3B vertices[8] = {{10, 10, 10}, {10, 0, 10}, {10, 10, 0}, {10, 0, 0},
  ...
  int chunkIndex =
      (removeBlock.x / WIDTH) + (removeBlock.z / WIDTH) * TOTALCHUNKWIDTH;
  ...
  int blockRotation = blockData[blockIndexR] &
  ...
  for (int i = 0; i < 6; i++) {
    ...
    if (newX > 0 && newX < WIDTH * TOTALCHUNKWIDTH && newY > 0 &&
      ...
      int blockRotation2 =
          ...
          (blockTypes[currentBlockType2].diffrentRotationAmount - 1);
      ...
      if (blockTypes[currentBlockType2]
                  .otherBlocksRenderFace[blockRotation2][blockSideIndex] == 0 ||
        ...
        screenPoint v1 = pointToScreenCorrdinates(
            (vertices[faces[i].x].x + removeBlock.x * 10) * 100,
            (vertices[faces[i].x].y + removeBlock.y * 10) * 100,
            (vertices[faces[i].x].z + removeBlock.z * 10) * 100);
        screenPoint v2 = pointToScreenCorrdinates(
            (vertices[faces[i].y].x + removeBlock.x * 10) * 100,
            (vertices[faces[i].y].y + removeBlock.y * 10) * 100,
            (vertices[faces[i].y].z + removeBlock.z * 10) * 100);
        screenPoint v3 = pointToScreenCorrdinates(
            (vertices[faces[i].z].x + removeBlock.x * 10) * 100,
            (vertices[faces[i].z].y + removeBlock.y * 10) * 100,
            (vertices[faces[i].z].z + removeBlock.z * 10) * 100);
        screenPoint v4 = pointToScreenCorrdinates(
            (vertices[faces[i].w].x + removeBlock.x * 10) * 100,
            (vertices[faces[i].w].y + removeBlock.y * 10) * 100,
            (vertices[faces[i].w].z + removeBlock.z * 10) * 100);
            ...
            if ((v1.y > 0 && v1.y < resY) || (v2.y > 0 && v2.y < resY) ||
              ...
              if (pixelSize == 2) {
                renderTriangleTexturedBlackTransNBPX2T(
                    V1_, V2_, V3_, AT, BT, CT, Depth, 16, 16,
                    &textures2[(111 + destroyIndex) * 256]);
                renderTriangleTexturedBlackTransNBPX2T(
                    V1_, V3_, V4_, AT, CT, DT, Depth, 16, 16,
                    &textures2[(111 + destroyIndex) * 256]);
              } else {
