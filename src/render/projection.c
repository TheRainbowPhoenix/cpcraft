/* src/render/projection.c - Particle rendering, 3D-to-2D projection, object
 * move/rotate */
#include "engine.h"
...
void renderParticle(float size, int mode, color_t color,
  ...
  for (int y = screenPixel.y - sizeOfLoopP2 / 2;
       y < screenPixel.y + sizeOfLoopP2 / 2; y += 2) {
    for (int x = screenPixel.x - sizeOfLoopP2 / 2;
         x < screenPixel.x + sizeOfLoopP2 / 2; x += 2) {
      if (x > 0 && x < resX && y > 0 && y < resY) {
        int distanceFromCenter;
        // switch (parti.mode)
        //{
        //     case 0:
        //     distanceFromCenter = calculateDistance(parti.position.x,
        //     parti.position.y, x, y); break; case 1: distanceFromCenter =
        //     abs_int(parti.position.x-x)+abs_int(parti.position.y-y); break;
        //     case 2:
...
void addRotateObjectNS(int indexGet, int indexSet, Vector3I center, float pitch,
                       float yaw) {
  float cYaw = myCosine(ToRadians(yaw));
  float sYaw = mySine(ToRadians(yaw));
  float cpitch = myCosine(ToRadians(pitch));
...
/* TODO: move me !! */
void createParticle(Vector3 position, float size, color_t color,
                    char transparency, char mode, int totalTime,
                    Vector3 velocity, bool gravity) {
  int index = -1;
...
void deleteParticle(int index) { allparticles[index].used = false; }