...
/* simulator/include/sdk/os/input.h
 *
 * Drop-in replacement for the HHK3 SDK's <sdk/os/input.h>.
 *
 * Same enums and struct layout, but Input_GetKeyState() reads from the
 * simulator's keyboard map (which is filled by the SDL event loop).
 * GetInput() is also implemented but most apps use Input_GetKeyState.
 */
#pragma once
...
struct __attribute__((packed)) Input_Event {
    ...
    union {
        ...
        struct {
            ...
            int32_t p1_x;
            int32_t p1_y;
            uint16_t adc_x1, adc_y1, adc_z1, adc_z2, adc_x2, adc_y2, adc_gh, adc_dm;
        } touch_single;
        struct {
            enum Input_TouchEventType direction : 32;
            int32_t p1_x, p1_y, p1_z, p2_x, p2_y, p2_z;
            uint32_t distance;
            uint16_t adc_x1, adc_y1, adc_z1, adc_z2, adc_x2, adc_y2, adc_gh, adc_dm;
        } touch_multi;
    } data;
...
extern bool (*Input_GetKeyStateU)(uint16_t *scancode);
static inline bool Input_GetKeyState(enum Input_Scancode scanCode) {
