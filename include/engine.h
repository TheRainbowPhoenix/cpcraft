 ...
 *     block_types.h    - Block type definitions (19 types: vertices, faces,
 * normals) items.h          - Item definitions (100 items with name, stack,
 * tool, etc.) blocks.h         - Block property definitions (73 blocks: destroy
 * time, drops, etc.) misc_data.h      - Missing icon + swap helper
 *
 *   src/
 ...
 *     save.c           - Save/load (world, chunks, chests, entities, player,
 * settings) misc.c           - Performance log, map view, entity reach game.c
 * - start_program + update (chunk loading) render_object.c  - Main game loop
 * (renderObject)
 *
 *   src/render/
 *     drawing.c        - Screen clear, sprite copy helpers
 *     skybox.c         - Skybox construction
 *     triangles.c      - Triangle/line rasterizers (textured, solid,
 * transparent) text.c           - Text rendering projection.c     - Particle
 * rendering, 3D-to-2D projection, object move/rotate effects.c        -
 * Crosshair + block destruction rendering screen_effects.c - Underwater + menu
 * visual effects
 *
 *   src/world/
...
void CopySpriteHeart(color_t *sprite, int x, int y, int width, int height,
                     color_t transparentColor, int maxX);
void CopySpriteNbitMasked(const unsigned char *data, int x, int y, int width,
                          int height, const color_t *palette, color_t maskColor,
                          unsigned int bitwidth);
void CopySpriteIcon(color_t *sprite, int x, int y, int width, int height,
                    color_t transparentColor);
...
void renderTriangleFast(Vector2S vertexA, Vector2S vertexB, Vector2S vertexC,
                        Vector2S texA, Vector2S texB, Vector2S texC,
                        const unsigned short depth, const int texWidth,
                        const int texHeight, const int textureIndex);
void renderTriangle(Vector2S *vertexA, Vector2S *vertexB, Vector2S *vertexC,
                    const color_t color, const unsigned short depth,
                    int brightness);
void renderTriangleTexturedNBT(Vector2S vertexA, Vector2S vertexB,
                               Vector2S vertexC, Vector2S texA, Vector2S texB,
                               Vector2S texC, const unsigned short depth,
                               const int texWidth_, const int texHeight_,
                               const color_t *textureAdress);
void renderSkybox(Vector2S vertexA, Vector2S vertexB, Vector2S vertexC,
                  Vector2S texA, Vector2S texB, Vector2S texC);

/* text.c */
void renderText(int x, int y, const char *text);
void renderTextColord(int x, int y, const char *text, color_t color);
void renderTextLargeColord(int x, int y, const char *text, color_t color,
                           color_t backGroundColor);
void renderTextUpdate(int x, int y, int stop, const char *text);

/* projection.c */
void renderParticle(float size, int mode, color_t color,
                    screenPoint screenPixel, int fallof);
...
void addRotateObject(int indexGet, int indexSet, Vector3I center, float pitch,
                     float yaw);
void setRotateObject(int indexGet, int indexSet, Vector3I center, float pitch,
                     float yaw);
...
unsigned int generate_random3D(unsigned int seed, int range, int x, int y,
                               int z);
...
void drawMinecraftFont(int x1, int x2, int y, bool center, int textLength,
                       char text[], color_t textColor, color_t textColorShadow);
...
void makeItem(int posX, int posY, int posZ, float velocityX, float velocityY,
              float velocityZ, int itemIndex);
...
void makeItemLoad(int posX, int posY, int posZ, float velocityX,
                  float velocityY, float velocityZ, int itemIndex, int slot);
